...
│   └── references/                # 参考文档
│       └── sources.md             # 信息源列表
├── github-trending/               # GitHub 开源项目趋势追踪
│   ├── SKILL.md                   # 主指令文件
│   ├── _meta.json                 # 元信息
│   └── references/                # 参考文档
...
│   ├── SKILL.md                   # 主指令文件
│   ├── _meta.json                 # 元信息
│   ├── templates/                 # handoff / knowledge 模板
│   └── references/                # 判定细则与示例
├── .gitignore
└── README.md
```
...
| [session-checkpoint](session-checkpoint/) | 会话检查点自动落盘 —— 判断对话是否值得持久化，分开写入任务交接（`.agent/handoffs/`）与长期知识（`.agent/knowledge/`），避免隔天续聊遗忘或跑偏 | v1.0.0 |
...
- 技术方案定案、排查结论、未完成交接，或说 **"按之前方案实施"** / **"继续上次"** → 触发 `session-checkpoint`
